...
<head>
	<body>
		<H1 align="center" style="font-family: Cambria, 'Hoefler Text', 'Liberation Serif', Times, 'Times New Roman', 'serif';color: rgba(16,35,247,1.00)" class="animated jackInTheBox">D.C.A.R RANKING</H1>
		<H4 align="center">Total Kunjungan Perbulan</H4>
...
<table class="table-responsive" width="100%" style="margin-bottom: 20px;font-family: Baskerville, 'Palatino Linotype', Palatino, 'Century Schoolbook L', 'Times New Roman', 'serif';font-size: 20px;font-style: oblique">
  ...
  $tahun= date('Y');
  $no=1;
  $jumlahSP= "SELECT nama_kolektor, COUNT( * ) AS total FROM laporan_penagihan WHERE month(tanggal_penagihan)=$bulan and year(tanggal_penagihan)=$tahun and media_penagihan='Kunjungan' GROUP BY nama_kolektor ORDER BY total DESC LIMIT 10";  
  $sql = mysqli_query($con, $jumlahSP);
  while($data = mysqli_fetch_array($sql)){
    echo "<tr>";
	echo "<td style='text-align: center' class='animated rollIn'>".$no."</td>";
	echo "<td style='text-align: center' class='animated rollIn'>".$data['nama_kolektor']."</td>";
