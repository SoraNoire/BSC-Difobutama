$(document).ready(function(){
	...
	$("#btn-simpan").click(function(){ // Ketika tombol simpan di klik
		...
		data.append('kode_lappenagihan', $("#kode_lappenagihan").val());
		data.append('klasifikasi_nasabah', $("#klasifikasi_nasabah").val());
		data.append('nama_nasabah', $("#nama_nasabah").val());
		data.append('media_penagihan', $("#media_penagihan").val()); // Ambil data nis
		data.append('no_tlp_n', $("#no_tlp_n").val()); // Ambil data nama
		data.append('alamat_nasabah', $("#alamat_nasabah").val()); // Ambil data jenis kelamin
		data.append('lokasi_lappenagihan', $("#lokasi_lappenagihan").val()); // Ambil data telepon
		...
		$.ajax({
			url: 'proses_simpan_lappenagihan.php', // File tujuan
	...
	$("#btn-ubah").click(function(){ // Ketika tombol ubah di klik
		...
		$.ajax({
			url: 'proses_ubah_lappenagihan.php', // File tujuan
			type: 'POST', // Tentukan type nya POST atau GET
			data: data, // Set data yang akan dikirim
			processData: false,
			contentType: false,
	...
	$("#btn-hapus").click(function(){ // Ketika tombol hapus di klik
		// Buat variabel data untuk menampung data hasil input dari form
		var data = new FormData();
		data.append('kode_lappenagihan', $("#data-kode_lappenagihan").val()); // Ambil data nis
		
		$("#loading-hapus").show(); // Munculkan loading hapus
		
		$.ajax({
			url: 'proses_hapus_lappenagihan.php', // File tujuan
			type: 'POST', // Tentukan type nya POST atau GET
			data: data, // Set data yang akan dikirim
			processData: false,
			contentType: false,
	...
	$('#form-modal').on('hidden.bs.modal', function (e){ // Ketika Modal Dialog di Close / tertutup
		...
		$("#kode_lappenagihan").removeAttr('readonly'); // Enable textbox nis
	});
});
