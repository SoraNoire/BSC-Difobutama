$(document).ready(function(){
	...
	$("#btn-simpan").click(function(){ // Ketika tombol simpan di klik
		...
		data.append('kode_sp', $("#kode_sp").val());
		data.append('uraian_aktivitas', $("#uraian_aktivitas").val());
		data.append('nama_nasabah', $("#nama_nasabah").val()); // Ambil data nis
		data.append('no_tlp_nasabah', $("#no_tlp_nasabah").val()); // Ambil data nama
		data.append('alamat_nasabah', $("#alamat_nasabah").val()); // Ambil data jenis kelamin
		data.append('lokasi_sp', $("#lokasi_sp").val()); // Ambil data telepon
		...
		$.ajax({
			url: 'proses_simpan_sp.php', // File tujuan
	...
	$("#btn-ubah").click(function(){ // Ketika tombol ubah di klik
		...
		$.ajax({
			url: 'proses_ubah_sp.php', // File tujuan
			type: 'POST', // Tentukan type nya POST atau GET
			data: data, // Set data yang akan dikirim
			processData: false,
			contentType: false,
	...
	$("#btn-hapus").click(function(){ // Ketika tombol hapus di klik
		// Buat variabel data untuk menampung data hasil input dari form
		var data = new FormData();
		data.append('kode_sp', $("#data-kode_sp").val()); // Ambil data nis
		
		$("#loading-hapus").show(); // Munculkan loading hapus
		
		$.ajax({
			url: 'proses_hapus_sp.php', // File tujuan
			type: 'POST', // Tentukan type nya POST atau GET
			data: data, // Set data yang akan dikirim
			processData: false,
			contentType: false,
	...
	$('#form-modal').on('hidden.bs.modal', function (e){ // Ketika Modal Dialog di Close / tertutup
		...
		$("#kode_sp").removeAttr('readonly'); // Enable textbox nis
	});
});
