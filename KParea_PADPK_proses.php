...
<form method="post" action="KParea_PADPK_save_proses.php?kode_permintaan=<?php echo $kode_permintaan; ?>" enctype="multipart/form-data">
	  <table style="font-size: 18px">
		  <tr>
			  <td style="padding-top: 30px"><u><b>Informasi Pelaksana</b></u></td>
		  </tr>
		  <tr>
			  <td style="padding-right: 30px">Pilih Analis Pelaksana</td>
			  <td>:</td>
			  <td style="padding-left: 10px">
			  <select name="nama_analis" class="form-control">
			  <option value="-">--- Pilih Pxelaksana ---</option>
				<?php
					$result99=mysqli_query($con, "select * from central_data where nik='$session_id'")or die('Error In Session');
					$row299=mysqli_fetch_array($result99);
					$jabatan99="Analis Kredit";
					$kantor99=$row299['kantor'];
					$penempatan99=$row299['penempatan'];
				?>
				<?PHP
					$querydropdown = mysqli_query($con,"SELECT * FROM central_data WHERE jabatan='$jabatan99' AND penempatan='$penempatan99' AND kantor='$kantor99' order by nama_karyawan") or die (mysqli_error());
					while ($rowdropdown = mysqli_fetch_array($querydropdown)) {
					$namapelaksana = strip_tags($rowdropdown['nama_karyawan']);
				?>
			<option>
				<?PHP echo $namapelaksana;?></option>
				<?PHP }?>
			</select>
		  	</td>
		  </tr>  
	  ...
	  <br></br>
      <input type="submit" class="btn btn-success" value="Lanjutkan Ke Analis">
      <a href="KParea_PADPK.php" class="btn btn-warning" onclick="javascript:history.back()"><span class="glyphicon glyphicon-remove-sign"> Cancel</span></a>
</form>
  </div>
