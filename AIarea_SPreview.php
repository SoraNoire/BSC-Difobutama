...
$("#nama_ao").html(msg);
	...
	<body>
		...
		$kantor=$row['kantor'];
		?>
		<form action="AIarea_SPreview.php" method="post" name="postform">
...
<option></option>
		...
		<?php
			...
			<script language="JavaScript">
				...
				document.location='AIarea_SPreview.php';
			</script>
			<?php
			}
			...
			<?php
			$query=mysqli_query($con, "SELECT * FROM sales_plan WHERE kantor='$realkantor' and tanggal_sp between '$tanggal_awal' and '$tanggal_akhir'");
			}
			elseif(empty($nama_ao)){
			?><i><b>Informasi : </b> Hasil pencarian data berdasarkan periode Tanggal <b><?php echo $_POST['tanggal_awal']?></b> s/d <b><?php echo $_POST['tanggal_akhir']?></b></i>
			<?php
			$query=mysqli_query($con, "SELECT * FROM sales_plan WHERE kantor='$realkantor' and penempatan='$realpenempatan' and tanggal_sp between '$tanggal_awal' and '$tanggal_akhir'");
			}	
			else
			{
			?><i><b>Informasi : </b> Hasil pencarian data berdasarkan periode Tanggal <b><?php echo $_POST['tanggal_awal']?></b> s/d <b><?php echo $_POST['tanggal_akhir']?></b></i>
			<?php
			$query=mysqli_query($con, "SELECT * FROM sales_plan WHERE id_ao='$nama_ao' and tanggal_sp between '$tanggal_awal' and '$tanggal_akhir'");
			}
		?>
		</p>
        ...
        <table class="table table-bordered" width="100%">
          <tr>
            ...
            <tr style="font-family: 'Gill Sans', 'Gill Sans MT', 'Myriad Pro', 'DejaVu Sans Condensed', Helvetica, Arial, 'sans-serif'">
              ...
              <td class="align-middle"><a href="<?php echo $data['lokasi_sp']; ?>" target="_blank">Klick Here</a></td>
