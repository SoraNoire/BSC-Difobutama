...
$bulan= date('m');
  $tahun= date('Y');
  $no=1;
  $jumlah= "SELECT rating, COUNT( * ) AS total FROM ratingmarcociledug GROUP BY rating"; 
  $rata2= "SELECT AVG(rating) AS ratarata from ratingmarcociledug";
...
<head>
	...
	function drawChart() {
		...
		var chart = new google.visualization.BarChart(document.getElementById('chart_ratingmarcocabang'));
...
<body>
    ...
    <div><input class="rating" min=0 max=5 step=1 value="<?php echo $dataAVG['ratarata'];?>" data-size="md" data-stars="5" name="rating" id="rating2" readonly></div>
	<!--Div yang akan menampilkan chart-->
    <div id="chart_ratingmarcocabang"></div>
	<div>
