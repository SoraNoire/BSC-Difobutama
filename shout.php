...
$db_username = 'dmballan';
$db_password = 'SR@DM-bsc$123';
$db_name = 'dmballan_ballancescorecard';
$db_host = 'localhost';
...
if($_POST)
{
	...
	if(isset($_POST["message"]) &&  strlen($_POST["message"])>0)
	{
		...
		mysqli_query($sql_con,"DELETE FROM shout_box WHERE id NOT IN (SELECT * FROM (SELECT id FROM shout_box ORDER BY id DESC LIMIT 0, 100) as sb)");
	}
	elseif($_POST["fetch"]==1)
	{
		$results = mysqli_query($sql_con,"SELECT user, message, date_time FROM (select * from shout_box ORDER BY id DESC LIMIT 100) shout_box ORDER BY shout_box.id ASC");
