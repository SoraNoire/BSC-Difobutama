...
$.ajax({
url: "DIREKSIarea_ambilnamaanalis.php",
data: "penempatan="+penempatan,
cache: false,
success: function(msg){
$("#nama_analis").html(msg);
}
});
});
});
	...
	<body>
		...
		$kantor=$row['kantor'];
		?>
		<form action="DIREKSIarea_DAARreview.php" method="post" name="postform">
			<p align="center"><font color="orange" size="10"><b>Laporan D.A.A.R</b></font></p><br />
			<table border="0">
<tr>
<td>Pilih Kantor</td>
<td>
...
<tr>
<td>Pilih Nama Analis </td>
<td>
<select name="nama_analis" id="nama_analis">
<option></option>
</select>
</td>
</tr>
		...
		<?php
			...
			<script language="JavaScript">
				...
				document.location='DIREKSIarea_DAARreview.php';
			</script>
			<?php
			}
			...
			<?php
			$query=mysqli_query($con, "SELECT * FROM daar WHERE kantor='$realkantor' and tanggal between '$tanggal_awal' and '$tanggal_akhir'");
			}
			elseif(empty($nama_analis)){
			?><i><b>Informasi : </b> Hasil pencarian data berdasarkan periode Tanggal <b><?php echo $_POST['tanggal_awal']?></b> s/d <b><?php echo $_POST['tanggal_akhir']?></b></i>
			<?php
			$query=mysqli_query($con, "SELECT * FROM daar WHERE kantor='$realkantor' and penempatan='$realpenempatan' and tanggal between '$tanggal_awal' and '$tanggal_akhir'");
			}	
			else
			{
			?><i><b>Informasi : </b> Hasil pencarian data berdasarkan periode Tanggal <b><?php echo $_POST['tanggal_awal']?></b> s/d <b><?php echo $_POST['tanggal_akhir']?></b></i>
			<?php
			$query=mysqli_query($con, "SELECT * FROM daar WHERE id='$nama_analis' and tanggal between '$tanggal_awal' and '$tanggal_akhir'");
			}
		?>
		</p>
      ...
      <div class="table-responsive" style="width: 100%">
        <table class="table table-bordered" width="100%">
          <tr>
            ...
            <th class="text-center text-nowrap">Nama Analis</th>
            <th class="text-center text-nowrap">Kode D.A.A.R</th>
            <th class="text-center text-nowrap">Uraian D.A.A.R</th>
            <th class="text-center text-nowrap">Jenis Kegiatan</th>
            ...
            <th class="text-center text-nowrap">No Telp Nasabah</th>
            ...
            <tr style="font-family: 'Gill Sans', 'Gill Sans MT', 'Myriad Pro', 'DejaVu Sans Condensed', Helvetica, Arial, 'sans-serif'">
              ...
              <td class="align-middle text-nowrap "><?php echo date("d/M/Y",strtotime($data['tanggal'])); ?></td>
              <td class="align-middle text-nowrap "><?php echo date("H:i:s a",strtotime($data['jam'])); ?></td>
              <td class="align-middle text-nowrap "><?php echo $data['nama']; ?></td>
              <td class="align-middle text-nowrap text-center"><?php echo $data['kode_kegiatan']; ?></td>
              ...
              <td class="align-middle"><a href="<?php echo $data['lokasi']; ?>" target="_blank">Click Here</a></td>
			  <td class="align-middle"><img src="imgdaar/<?php echo $data['foto']; ?>" width="80" height="80"></td>
        </td>
