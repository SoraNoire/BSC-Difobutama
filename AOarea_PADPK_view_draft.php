...
<div class="table-responsive" border="1" width="100%" style="margin-bottom: 20px">
 <table class="table table-bordered" style="font-family: Segoe, 'Segoe UI', 'DejaVu Sans', 'Trebuchet MS', Verdana, 'sans-serif'" width="100%">
  ...
  <?php
    ...
    $kantor= $row['kantor'];
	$owner = $row['nama_karyawan'];
		
  $query = "SELECT * FROM permohonan_analisa_kredit WHERE status='DRAFT' and nama_ao='$owner'"; 
  ...
  while($data = mysqli_fetch_array($sql)){
    ...
    echo "<td align='center'><a href='AOarea_PADPK_viewer.php?kode_permintaan=".$data['kode_permintaan']."' class='btn btn-default'><span class='glyphicon glyphicon-eye-open'></span></a></td>";
    echo "<td align='center'><a href='AOarea_PADPK_edit.php?kode_permintaan=".$data['kode_permintaan']."' class='btn btn-warning'><span class='glyphicon glyphicon-edit'></span></a></td>";
    echo "<td align='center'><a href='AOarea_PADPK_delete.php?kode_permintaan=".$data['kode_permintaan']."' class='btn btn-danger'><span class='glyphicon glyphicon-erase'></span></a></td>";
    echo "</tr>";
  }
  ?>
	</table>
