...
function smtp_mail($to, $subject, $message, $from_name, $from, $cc, $bcc, $debug=false) {
  ...
  $mail->SMTPAuth     = true;                        				
  $mail->Host 		  = 'dm-ballancescorecard.com;101.50.1.15';  // Masukkan Server SMTP
  $mail->Port 		  = 465;                                      // Masukkan Port SMTP
  $mail->SMTPSecure   = 'ssl';                                    // Masukkan Pilihan Enkripsi ( `tls` atau `ssl` )
  $mail->Username 	= 'support@dm-ballancescorecard.com';                // Masukkan Email yang digunakan selama proses pengiriman email via SMTP
  $mail->Password 	= 'silentripper';        						          // Masukkan Password dari Email tsb
  $default_email_from       = 'support@dm-ballancescorecard.com';        // Masukkan default from pada email
  $default_email_from_name  = 'SUPPORT BSC';           // Masukkan default nama dari from pada email
