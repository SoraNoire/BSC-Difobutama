...
$status 	 = 'rejected';
$zero    	 = '';
$informasi	 = 'Data Ditolak karna alasan tertentu';
...
Hai, Mohon Maaf Data Anda Dinyatakan Tidak Layak Oleh Bagian Analis. Tetap Semangat Ya, Berikut Informasinya :    <br><br>
...
if(($kode_permintaan && $nama_ao && $nama_nasabah && $alamat_nasabah && $no_tlp_nasabah && $plafond)){
	$query = "UPDATE permohonan_analisakredit SET status='".$status."' WHERE kode_permintaan='".$kode_permintaan."'";
	$sql = mysqli_query($con, $query);
	
	if($query){	
		$insert = mysqli_query($con,"INSERT INTO padpkhistory_data VALUES('$kode_permintaan','$tanggal','$jam','$informasi','$nama_pemohon','$keterangan')");
		 ?>
